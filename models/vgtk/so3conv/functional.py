...
def inter_so3conv_feat_grouping(inter_idx, inter_w, feats):
    # [nb, np, nn] x [nb, np, na, ks, nn] x [b, c_in, np+1, na] -> [nb, c_in, ks, np, na]
...
def inter_spconv_grouping_ball(xyz, stride, radius, n_neighbor, lazy_sample=True):
    ...
    # [b, 3, p2, nn] x [b, 3, p2, 1] -> [b, 3, p2, nn]
    grouped_xyz = grouped_xyz - sample_xyz.unsqueeze(3)
    return grouped_xyz, ball_idx, idx, sample_xyz

def graph_inter_spconv_grouping(xyz, lazy_sample=True):
    # This is where graph needs to be constructed
    group_idx = torch.arange(xyz.shape[2], device=xyz.device).unsqueeze(0).unsqueeze(0).repeat(xyz.shape[0], xyz.shape[2], 1)
    # [b, 3, 1, p1] x [b, 3, p1, 1] -> [b, 3, p1, p1]
    grouped_xyz = xyz.unsqueeze(2) - xyz.unsqueeze(3)
    return grouped_xyz, group_idx, None, xyz
...
def graph_inter_so3conv_grouping(xyz, feats, 
    ...
    '''
    if inter_idx is None: # Here!
        grouped_xyz, inter_idx, sample_idx, new_xyz = graph_inter_spconv_grouping(xyz, lazy_sample)
