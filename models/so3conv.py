...
# [b, c1, p1, a] -> [b, c1, k, p2, a] -> [b, c2, p2, a]
class GraphInterSO3ConvBlock(nn.Module):
    def __init__(self, dim_in, dim_out, kernel_size,
                 radius, sigma, kanchor=60,
                 lazy_sample=None, norm=None, activation='relu', pooling='none', dropout_rate=0):
        super(GraphInterSO3ConvBlock, self).__init__()
        ...
        pooling_method = None if pooling == 'none' else pooling
        self.conv = sptk.GraphInterSO3Conv(dim_in, dim_out, kernel_size,
                                      radius, sigma, kanchor=kanchor,
                                      lazy_sample=lazy_sample, pooling=pooling_method)
        self.norm = norm(dim_out, affine=False)
        if activation is None:
            self.relu = None
...
class BasicSO3ConvBlock(nn.Module):
    def __init__(self, params):
        ...
        for param in params:
            ...
            elif param['type'] == 'separable_graph_block':
                conv = GraphSO3ConvBlock(param['args'])
            else:
    ...
    def forward(self, x, inter_idx=None, inter_w=None):
        ...
        if graph_conv:
            return inter_idx, inter_w, x
        return x

    def get_anchor(self):
        return torch.from_numpy(sptk.get_anchors())

class GraphSO3ConvBlock(nn.Module):
    def __init__(self, params):
        super(GraphSO3ConvBlock, self).__init__()
        ...
        self.inter_conv = GraphInterSO3ConvBlock(**params)
...
class SO3OutBlockRT(nn.Module):
    def __init__(self, params, norm=None, pooling_method='mean', global_scalar=False, use_anchors=False,
        ...
        if self.feat_mode_num < 2:
            self.regressor_layer = nn.Conv1d(mlp[-1],9,(1))
        else:
            self.regressor_layer = nn.Conv1d(mlp[-1], 9 * num_heads, (1))
    ...
    def forward(self, x, anchors=None):
        ...
        if self.global_scalar:
            ...
            if self.use_anchors:
                y_t = (torch.matmul(anchors,#Edited this part.unsqueeze(1)
        ...
        # regressor
        output = {}
        y = self.regressor_layer(x_out).reshape(nb, self.num_heads, -1, na)  # [B, num_heads, 4, A]
        output['1'] = confidence  #
        output['R'] = y
        output['T'] = y_t
...
class SO3FrameOutBlockRT(nn.Module):
    def __init__(self, params, norm=None, pooling_method='mean', global_scalar=False, use_anchors=False,
                 feat_mode_num=60, num_heads=1):
        super(SO3FrameOutBlockRT, self).__init__()
        ...
        skinning_weights = np.load('/equi-pose/data/ours_Synthetic/skinning_weights.npy').astype(np.float32)
        self.register_buffer('skinning_weights', torch.from_numpy(skinning_weights))
    ...
    def forward(self, x, anchors=None):
        ...
        for lid, linear in enumerate(self.linear):
            ...
            if self.norm is not None:
                x_out = self.norm[lid](x_out)
            x_out = F.relu(x_out)
        x_out = x_out.view(-1, nf, *x_out.shape[1:])
        x_out = x_out.mean(dim=1)
        x_xyz = x_xyz.mean(dim=1)
        ...
        if self.pooling_method == 'mean':
            ...
            x_out = x_out.unsqueeze(3) * (self.skinning_weights != 0)
            x_out = x_out.max(2)[0]
        elif self.pooling_method == 'pointnet':
            x_in = sptk.SphericalPointCloud(x.xyz, x_out, None)
            x_out = self.pointnet(x_in)
        ...
        x_out = x_out.view(nb, nc, nj * na)
        attention_wts = self.attention_layer(x_out)  # [B, num_heads, A]
        confidence = F.softmax(attention_wts * self.temperature, dim=2)
        # regressor
        output = {}
        y = self.regressor_layer(x_out).reshape(nb, self.num_heads, nj, na).reshape(nb, 9, nf, nj, na).transpose(1, 3).reshape(nb, nf*nj, 9, na)# [B, num_heads, ]
        output['1'] = confidence  #
        output['R'] = y
        output['T'] = y_t
